...
public class ColeccionSectoresFabrica
{
    protected Robot[] T;
    protected int cantOcupados;

    //--------------------------------------------------------
    //---- Constructor
    //--------------------------------------------------------

    public ColeccionSectoresFabrica(int max) 
    {
        /*
    ...
    public void comprimir()
    {
        ...
        while (i < cantSectoresOcupados()) {
            if (T[i] == null) {
                cantEliminados++;
                T[i] = T[i + cantEliminados];                
            }
            i++; 
        }
        cantOcupados -= cantEliminados;
    }    
    ...
    public boolean estaRobot(Robot r)
    {
        ...
        int i = 0; 
        boolean esta = false;
        while (i < cantSectoresOcupados() && !esta ){
            esta = (T[i] == r);
            i++; 
        }
        return esta;
    ...
    public Robot robotSector(int s)
    {
         ...
         * Requiere 0 <= s < cantSectoresOcupados()
         */
        return T[s];
    }
    ...
    public int cantMasAutos(int a)
    {
         ...
         */
        int cont =0;
        for (int i=0;i<cantSectoresOcupados();i++) {
            if (T[i] != null) {
                if(T[i].cantAutos() > a) {
                    cont++;
                }
    ...
    public boolean dosConsecutivosMaxEnergia()
    {
        boolean dos = false;
        int i = 0;
        while (i < cantSectoresOcupados() && !dos)
        {
